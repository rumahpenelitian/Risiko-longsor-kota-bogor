...
function updateAreaInfo(kecamatan, kelurahan) {
    ...
    if (kecamatan) {
        let infoText = `<div class="area-detail">
            <p><strong>Kecamatan:</strong> ${formatKecamatanName(kecamatan)}</p>`;
        if (kelurahan) {
            ...
            if (riskResult) {
                infoText += `
                    <p><strong>Tingkat Risiko Longsor:</strong> ${formatSetName(riskResult.category)}</p>
                    <p><strong>Nilai Risiko:</strong> ${riskResult.value.toFixed(2)}</p>
                `;
            }
        }
        infoText += '</div>';
        ...
        areaInfo.innerHTML = 'Silakan pilih kecamatan dan kelurahan untuk melihat informasi detail.';
    }
}
    ...
    .then(data => {
        ...
        geojsonLayer = L.geoJSON(data, {
            ...
            onEachFeature: function(feature, layer) {
                ...
                if (riskResult) {
                    popupContent += `
                        <p><b>Tingkat Risiko Longsor:</b> ${formatSetName(riskResult.category)}</p>
                        <p><b>Nilai Risiko:</b> ${riskResult.value.toFixed(2)}</p>
                    `;
                }
        ...
        data.features.forEach(feature => {
            if (feature.properties.WADMKC) {
                const normalizedName = normalizeKecamatanName(feature.properties.WADMKC);
                kecamatanMap.set(normalizedName, feature.properties.WADMKC);
            }
        });

        const kecamatanSelect = document.getElementById('kecamatan');
        Array.from(kecamatanMap.keys()).sort().forEach(normalizedName => {
            const option = document.createElement('option');
            option.value = normalizedName;
            option.textContent = formatKecamatanName(kecamatanMap.get(normalizedName));
...
document.getElementById('kecamatan').addEventListener('change', function(e) {
    ...
    if (selectedKecamatan) {
        ...
        const kelurahanFeatures = geojsonData.features
            .filter(feature => {
                if (!feature.properties.WADMKC) return false;
                const normalizedFeatureKecamatan = normalizeKecamatanName(feature.properties.WADMKC);
        ...
        Array.from(uniqueKelurahan).sort().forEach(kelurahan => {
            const option = document.createElement('option');
        ...
        geojsonLayer.setStyle(feature => ({
            color: '#0000FF',
            weight: 2,
            fillOpacity: feature.properties.WADMKC && 
                        normalizeKecamatanName(feature.properties.WADMKC) === selectedKecamatan ? 0.3 : 0.1
        }));
        ...
        // Reset map view
        map.setView([-6.5971, 106.8060], 12);
    }
});
...
document.getElementById('kelurahan').addEventListener('change', function(e) {
    ...
    if (selectedKecamatan && selectedKelurahan) {
        ...
        geojsonLayer.setStyle(feature => {
            const isSelected = feature.properties.WADMKC && 
                             normalizeKecamatanName(feature.properties.WADMKC) === selectedKecamatan &&
                             feature.properties.NAMOBJ === selectedKelurahan;
            
            if (isSelected) {
                const riskResult = getRiskResult(kecamatanMap.get(selectedKecamatan), selectedKelurahan);
                return {
                    color: riskResult ? getRiskColor(riskResult.category) : '#0000FF',
                    weight: 2,
                ...
                const riskResult = getRiskResult(feature.properties.WADMKC, feature.properties.NAMOBJ);
                return {
                    color: riskResult ? getRiskColor(riskResult.category) : '#0000FF',
                    weight: 2,
                    fillOpacity: riskResult ? 0.6 : 0.1
...
function updateInputMembershipCharts(visualizations) {
    const chartContainer = document.getElementById('inputMembershipChart').getContext('2d');
...
function updateOutputMembershipChart(visualizations) {
    const chartContainer = document.getElementById('outputMembershipChart').getContext('2d');
...
function displayResults(results) {
    ...
    for (const [variable, values] of Object.entries(results.memberships)) {
        ...
        for (const [set, value] of Object.entries(values)) {
            membershipHtml += `<span class="membership-value" style="color: ${FuzzyLogic.getSetColor(set)}">
    ...
    const outputHtml = `
        <div class="output-value">
            <h5>Hasil Akhir:</h5>
            <p>Nilai Crisp: <strong>${results.crispOutput.toFixed(2)}</strong></p>
            <p>Kategori: <strong style="color: ${FuzzyLogic.getSetColor(results.outputCategory)}">
    ...
    let riskHtml = `<div class="risk-assessment" style="border-color: ${FuzzyLogic.getSetColor(results.outputCategory)}">`;
